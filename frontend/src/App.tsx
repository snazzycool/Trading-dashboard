...
function EmptyState({ active, connected, onStart }: { active: boolean; connected: boolean; onStart: () => void }) {
  return (
    <div className="flex flex-col items-center py-16 text-center">
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${active ? 'bg-blue-500/10' : 'bg-white/5'}`}>
        <Activity size={26} className={active ? 'text-blue-400 animate-pulse' : 'text-white/15'} />
      </div>
      {!connected ? (
        <><p className="text-gray-400 font-medium">Connecting…</p><p className="text-gray-600 text-sm mt-1">Please wait</p></>
      ) : !active ? (
        <>
          ...
          <button onClick={onStart} className="flex items-center gap-2 bg-green-500/15 hover:bg-green-500/25 text-green-400 border border-green-500/30 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all">
            <Play size={13} className="fill-current" />Start Scanner
          </button>
        </>
      ) : (
        <>
